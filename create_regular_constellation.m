...
function [path_data] = create_regular_constellation(n_r, m, orbiting_velocity, sparsity_factor, back_scattering) 

ind_aux = [1:sparsity_factor:n_r]';
path_data = ind_aux;
path_data_aux = [];
aux_ind = [1 : floor(n_r/m):n_r];
aux_ind = aux_ind(1:m);
for i = 1 : m
aux_vec = mod(floor(aux_ind(i) + orbiting_velocity*ind_aux), n_r)+1;
